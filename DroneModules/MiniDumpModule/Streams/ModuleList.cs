...
namespace MiniDumpModule.Streams
{
    public class ModuleList
    {
        ...
        public static List<MinidumpModule> parse(Directory.MINIDUMP_DIRECTORY dir, MiniDump minidump)
        {
            List<MinidumpModule> list = new List<MinidumpModule>();
            minidump.BinaryReader.BaseStream.Seek(dir.Offset, 0);
            byte[] chunk = minidump.BinaryReader.ReadBytes((int)dir.Size);
            ...
            using (BinaryReader ChunkReader = new BinaryReader(new MemoryStream(chunk)))
            {
                ...
                {
                    MinidumpModule module = parse_mod(mod, minidump.BinaryReader);
                    list.Add(module);
                }
            }
            return list;
